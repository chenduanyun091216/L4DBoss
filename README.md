...
- 内置深渊蓝、明亮和钛色灰三套界面主题；品牌动态光效、文字、弹窗和每套主题专属图标都会同步适配。
...
默认窗口尺寸为 `1250 × 730`，默认卡片宽度为 `168px`。顶部控件以这组默认尺寸为固定基准：

- 组合下拉框宽度等于一张默认卡片。
- 搜索框宽度等于两张默认卡片加一个卡片间距。
- 搜索框上方四个操作按钮完整覆盖相同宽度；像素不能整除时最多相差 `1px`。
- 创建 Mod 按钮与组合下拉框左侧对齐，并与顶部操作按钮保持一致的视觉宽度。
- 拖动窗口或调整卡片尺寸只改变布局位置与卡片本身，不会改变上述顶部控件长度。
...
测试覆盖冲突检测规则、创建 Mod 两层模态弹窗、固定顶部布局、标签气泡与复用、后台扫描响应性、预览缓存上限、自动封面生成和 JSON 原子保存。UI 测试可使用 Qt 离屏平台运行：
